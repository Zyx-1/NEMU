...
enum {
	NOTYPE = 256, EQ, NEQ, AND, OR, MINUS, POINTER, NUMBER, HEX, REGISTER, MARK
...
int dominant_operator(int l, int r){
	...
	for(i = l; i <= r; i++){
		...
		if(!flag) continue;
		if(tokens[i].priority <= mprio){
			mprio = tokens[i].priority;
			op = i;
		}
	}
	return op;
